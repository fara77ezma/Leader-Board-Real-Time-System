...
redis_client = redis.Redis(host='redis', # docker service name
                            port=6379,
                            decode_responses=True # to get string responses
                           )

def submit_score( request: SubmitScoreRequest,
    current_user: dict ,
    db: Session 
    ):
    ...
    try:
        ...
        if score <= current_best:
            return {"message": "Score submitted successfully.","score": score, "rank": redis_client.zrevrank(redis_key, user_id)+ 1} 
        else:
        
            redis_client.zadd(redis_key, {user_id: score})
            rank = redis_client.zrevrank(redis_key, user_id) 
            return {"message": "Score submitted successfully.","score": score, "rank": rank + 1}  # rank is 0-based
    except Exception as e:
...
def fetch_leaderboard(game_id: str,
    limit: int ,
    db: Session 
    ):
    ...
    redis_key = f"leaderboard:{game_id}"
    try:
        top_entries = redis_client.zrevrange(redis_key, 0, limit - 1, withscores=True) # Get top 'limit' entries in descending order with scores
