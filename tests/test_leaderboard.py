...
class TestSubmitScore:
    ...
    def test_new_high_score_is_added_to_redis(self, db_session, mock_leaderboard_user, mocker):
        ...
        result = submit_score(make_submit_request(score=150), make_current_user(), db_session)
    ...
    def test_lower_score_does_not_update_redis(self, db_session, mock_leaderboard_user, mocker):
        db_session.query.return_value.filter.return_value.first.return_value = mock_leaderboard_user
        mock_redis = mocker.patch("controllers.leaderboard.redis_client")
        mock_redis.zscore.return_value = 200  # existing best is 200
        mock_redis.zrevrank.return_value = 2  # rank 3 (0-indexed)

        result = submit_score(make_submit_request(score=100), make_current_user(), db_session)
    ...
    def test_equal_score_does_not_update_redis(self, db_session, mock_leaderboard_user, mocker):
        db_session.query.return_value.filter.return_value.first.return_value = mock_leaderboard_user
        mock_redis = mocker.patch("controllers.leaderboard.redis_client")
    ...
    def test_redis_failure_returns_error(self, db_session, mock_leaderboard_user, mocker):
        db_session.query.return_value.filter.return_value.first.return_value = mock_leaderboard_user
        mock_redis = mocker.patch("controllers.leaderboard.redis_client")
