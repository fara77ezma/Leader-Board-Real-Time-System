...
from fixtures.leaderboard_fixtures import current_user, make_submit_request, mock_leaderboard_user
...
class TestSubmitScore:
    def test_user_not_found_returns_error(self, db_session):
        ...
        result = submit_score(make_submit_request(), current_user(), db_session)

        assert result == {"error": "User not found."}

    def test_new_high_score_is_added_to_redis(self, db_session, mocker):
        db_session.query.return_value.filter.return_value.first.return_value = mock_leaderboard_user()
        ...
        result = submit_score(make_submit_request(score=150), current_user(), db_session)
    ...
    def test_lower_score_does_not_update_redis(self, db_session, mocker):
        db_session.query.return_value.filter.return_value.first.return_value = mock_leaderboard_user()
        mock_redis = mocker.patch("controllers.leaderboard.redis_client")
        mock_redis.zscore.return_value = 200  # existing best is 200
        mock_redis.zrevrank.return_value = 2  # rank 3 (0-indexed)

        result = submit_score(make_submit_request(score=100), current_user(), db_session)
    ...
    def test_equal_score_does_not_update_redis(self, db_session, mocker):
        db_session.query.return_value.filter.return_value.first.return_value = mock_leaderboard_user()
        mock_redis = mocker.patch("controllers.leaderboard.redis_client")
    ...
    def test_db_commit_failure_rolls_back(self, db_session, mocker):
        db_session.query.return_value.filter.return_value.first.return_value = mock_leaderboard_user()
...
class TestFetchLeaderboard:
    def test_returns_ranked_entries(self, db_session, mocker):
        ...
        alice = mock_leaderboard_user(user_id=1, username="alice")
        bob = mock_leaderboard_user(user_id=2, username="bob")
        db_session.query.return_value.filter.return_value.first.side_effect = [alice, bob]

        result = fetch_leaderboard("game_001", limit=10, db=db_session)

        assert result["game_id"] == "game_001"
        assert len(result["leaderboard"]) == 2
        assert result["leaderboard"][0] == {"rank": 1, "username": "alice", "score": 200.0}
        assert result["leaderboard"][1] == {"rank": 2, "username": "bob", "score": 150.0}
    ...
    def test_respects_limit_parameter(self, db_session, mocker):
        ...
        mock_redis.zrevrange.assert_called_once_with("leaderboard:game_001", 0, 4, withscores=True)
    ...
    def test_skips_entry_when_user_not_in_db(self, db_session, mocker):
        ...
        alice = mock_leaderboard_user(user_id=1, username="alice")
        db_session.query.return_value.filter.return_value.first.side_effect = [alice, None]
